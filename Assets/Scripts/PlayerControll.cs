...
public class PlayerControll : MonoBehaviour
{
    ...
    private void Move()
    {
        ...
        // アニメーション切り替え
        float currentSpeed = new Vector3(h,0,v).magnitude;
        animator.SetFloat("Speed",currentSpeed);

        // 足音
        if(move.sqrMagnitude > 0.1f && characterController.isGrounded)
        {
            stepTImer -= Time.deltaTime;
            if(stepTImer <= 0f)
            {
                stepTImer = stepInterval;
                footstep.PlayerFootsStep(transform.position);
            }
        }
        else
        {
            footstep.audioSource.Stop();
            stepTImer = 0.1f; // 停止時にリセット
        }
    }
